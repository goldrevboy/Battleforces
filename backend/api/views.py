...
from .helper import getRandomProblemByRating , getprob , check_solved
...
class ScoreboardView(generics.RetrieveUpdateAPIView):
    permission_classes = [IsAuthenticated]
    def put(self, request, contest_id):
        user = request.user
        profile = UserProfile.objects.get(user=user)
        ...
        ret_obj = []
        for i in teams:
            users = [ i.user1.user.username , i.user2.user.username , i.user3.user.username ]
            penalty = 0
            scored = 0
            teamid = i.id 
            ...
            for j in range(len(problem_data)):
                ws , fact , tim = check_solved(users,problem_date[j])
